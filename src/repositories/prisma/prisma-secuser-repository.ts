...
export class PrismaSecUserRepository implements SecUserRepository  {
    ...
    async encryptPassword(password: string, registrationDate: Date): Promise<string | null> {
        try {
    ...
    async decryptPassword(password: string, registrationDate: Date): Promise<string | null> {
